...
async function run(contents: string, skipVersionCheck = false) {
  return runFiles(abap, [{filename: "zfoobar_string_templates.prog.abap", contents}], {skipVersionCheck});
}
...
WRITE |{ lv_value STYLE = SCIENTIFIC }|.`;
  ...
  it("Output STYLE = SCIENTIFIC, fifty", async () => {
    const code = `
DATA lv_value TYPE f.
lv_value = 50.
WRITE |{ lv_value STYLE = SCIENTIFIC }|.`;
...
WRITE |{ lv_value STYLE = SCIENTIFIC }|.`;
  ...
  it.only("ALPHA formatting", async () => {
    const code = `
DATA lv_value TYPE n LENGTH 5.
lv_value = 5.
WRITE |{ lv_value ALPHA = OUT }|.
WRITE |{ lv_value ALPHA = IN }|.`;
