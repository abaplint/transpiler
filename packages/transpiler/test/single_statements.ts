...
describe("Single statements", () => {
  const tests = [
    ...
    {abap: "READ TABLE tab INDEX i INTO target.",          js: "await abap.statements.readTable(tab,{index: i,into: target});", skip: false},
    {abap: "READ TABLE tab INTO line WITH KEY field = 2.", js: "await abap.statements.readTable(tab,{into: line,withKey: async (i) => {return abap.compare.eq(i.field, constant_2);}});", skip: false},
    {abap: "READ TABLE tab INDEX i ASSIGNING <nam>.",      js: "await abap.statements.readTable(tab,{index: i,assigning: fs_nam_});",   skip: false},
    ...
    {abap: "RAISE EXCEPTION TYPE zcx_foobar EXPORTING foo = bar.", js: `throw await (new zcx_foobar()).constructor_({foo: bar});`, skip: false},
