...
describe("Single statements", () => {
  const tests = [
    ...
    {abap: "foo = |fo{ 2 }|.",                     js: "foo.set(new abap.types.String().set(`fo${abap.templateFormatting(constant_2)}`));", skip: false},
    {abap: "foo = `fo`.",                          js: "foo.set(new abap.types.String().set(`fo`));",     skip: false},
    ...
    {abap: "DATA foo TYPE STANDARD TABLE OF string.", js: `let foo = new abap.types.Table(new abap.types.String({qualifiedName: "STRING"}), {"withHeader":false,"type":"STANDARD","isUnique":false,"keyFields":[]}, "");`,         skip: false},
    ...
    {abap: "ASSIGN ('SDF') TO <fs2>.",                js: "abap.statements.assign({target: fs_fs2_, dynamicName: 'SDF', dynamicSource: (() => {try { return sdf; } catch {}})()});",    skip: false},
    {abap: "ASSIGN (blah) TO <fs2>.",                 js: "abap.statements.assign({target: fs_fs2_, dynamicName: blah.get(), dynamicSource: (() => {try { return eval(blah.get().toLowerCase().match(/\\w+/)[0]); } catch {}})()});",    skip: false},
    {abap: "ASSIGN (blah)=>(sdf) TO <fs2>.",          js: "abap.statements.assign({target: fs_fs2_, dynamicName: blah.get() + '=>' + sdf.get(), dynamicSource: (() => {try { return eval(blah.get().toLowerCase().match(/\\w+/)[0]); } catch {}})()});",    skip: false},
    ...
    {abap: "SELECT * FROM t100 INTO TABLE lt_result UP TO 2 ROWS.",
      js: `await abap.statements.select(lt_result, {select: "SELECT * FROM t100 UP TO " + constant_2.get() + " ROWS"});`},
    ...
    {abap: "SELECT * FROM sflight INTO CORRESPONDING FIELDS OF TABLE mt_sflight UP TO 10 ROWS ORDER BY carrid.",
      js: `await abap.statements.select(mt_sflight, {select: "SELECT * FROM sflight UP TO " + constant_10.get() + " ROWS ORDER BY carrid"});`},
...
await abap.Classes['KERNEL_SCAN_ABAP_SOURCE'].call({scan_abap_source: source, tokens_into: tokens, statements_into: statements, with_analysis: true, with_comments: true, with_pragmas: new abap.types.Character({length: 1}).set('*')});`},
    ...
    {abap: `DATA tab TYPE SORTED TABLE OF i WITH UNIQUE KEY table_line.`,
      js: `let tab = new abap.types.Table(new abap.types.Integer({qualifiedName: "I"}), {"withHeader":false,"type":"SORTED","isUnique":true,"keyFields":["TABLE_LINE"]}, "");`},
