...
describe("Single statements", () => {
  ...
  withKeySimple: {"program_ndx": lv_program_ndx}});`,   skip: false},
    ...
    {abap: "SELECT SINGLE * FROM t100 INTO ls_result.",
      js: `await abap.statements.select(ls_result, {select: "SELECT * FROM " + abap.dbo.schemaPrefix + "\\"" + abap.dbo.tablePrefix + "t100\\" UP TO 1 ROWS"});`},
    {abap: "SELECT * FROM t100 INTO TABLE lt_result ORDER BY msgnr.",
      js: `await abap.statements.select(lt_result, {select: "SELECT * FROM " + abap.dbo.schemaPrefix + "\\"" + abap.dbo.tablePrefix + "t100\\" ORDER BY msgnr"});`},
    {abap: "SELECT * FROM t100 INTO TABLE lt_result UP TO 2 ROWS.",
      js: `await abap.statements.select(lt_result, {select: "SELECT * FROM " + abap.dbo.schemaPrefix + "\\"" + abap.dbo.tablePrefix + "t100\\" UP TO " + new abap.types.Integer().set(2).get() + " ROWS"});`},
    ...
    {abap: "select count( * ) from /foo/bar into bar.",
      js: `await abap.statements.select(bar, {select: "SELECT count( * )  from " + abap.dbo.schemaPrefix + "\\"" + abap.dbo.tablePrefix + "/foo/bar\\""});`},
